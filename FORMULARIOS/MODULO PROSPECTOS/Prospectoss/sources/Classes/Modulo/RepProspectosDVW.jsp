<html>
<head>

</head>
<%=com.unify.nxj.awebServlet.Runtime.generateOnLoad(request, response, application)%>
<body onLoad="initPreview(false, -1, -1, 'file:///C:\Unify\NXJ/web-app/unify/images', 'file:///C:/Users/TI3-Provivienda/Desktop/VENTAS- PROSPECTOS/MODULO PROSPECTOS/Prospectoss/sources/Static_Content');window.initInternalJavaScriptComponents();" class="form" >

    <form
        binary_variables=""
        class="form"
        displayheight="430"
        displaywidth="1055"
        field_order=""
        form_height="430"
        form_width="1055"
        name="NXJForm"
        object_type="data_view"
        view="form">

        <div
            binary_variables=""
            view="div"
            style="position: absolute; top: 25px; left: 25px; width:1005px; height:380px">

            <div
                aud_on_entry="true"
                binary_variables=""
                bloom_filter="00000000000400042000000000000000"
                border-width="1"
                class="data_view"
                commands="reportecmd"
                connection="Connection1"
                displayheight="380"
                displayleft="25"
                displaytop="25"
                displaywidth="1005"
                fieldheight="380"
                fieldwidth="1005"
                foundation_type="Classes/Foundation/Data view"
                id=".top."
                key=".top."
                kind="data_region"
                name=".top."
                uid_prefix="itemsform_dataview1_1368038367945"
                view="data_region"
                style="border-width:1;
                    border-style:solid;
                    background-color: #cccccc">

                <div
                    binary_variables=""
                    view="div"
                    style="position: absolute; top: 153px; left: 341px; width:21px; height:21px">

                    <input
                        auto_accept="true"
                        binary_variables=""
                        checkbox_on_values="T"
                        checkbox_store_format="T/N"
                        class="checkbox"
                        displayheight="21"
                        displayleft="341"
                        displaytop="153"
                        displaywidth="21"
                        field_type="checkbox"
                        field_weight="299"
                        findable="false"
                        immediate="true"
                        key=".top:TodosLoProy."
                        name=".top:TodosLoProy."
                        original_name="checkbox1"
                        target_field=""
                        type="checkbox"
                        uid="parametros_form1_1369767389242_9aa357ed538e2cd1180f71a52d2b666790b6d2f4_1369775564470"
                        view="checkbox">
                </div>

                <div
                    binary_variables=""
                    view="div"
                    style="position: absolute; top: 209px; left: 341px; width:21px; height:21px">

                    <input
                        auto_accept="true"
                        binary_variables=""
                        checkbox_on_values="T"
                        checkbox_store_format="T/N"
                        class="checkbox"
                        displayheight="21"
                        displayleft="341"
                        displaytop="209"
                        displaywidth="21"
                        field_type="checkbox"
                        field_weight="599"
                        findable="false"
                        immediate="true"
                        key=".top:TodosLosVend."
                        name=".top:TodosLosVend."
                        original_name="checkbox1"
                        target_field=""
                        type="checkbox"
                        uid="parametros_form1_1369767389242_9aa357ed538e2cd1180f71a52d2b666790b6d2f4_1369775566771"
                        view="checkbox">
                </div>

                <div
                    binary_variables=""
                    view="div"
                    style="position: absolute; top: 325px; left: 4px; width:996px; height:50px">

                    <fieldset
                        binary_variables=""
                        border-width="0"
                        class="box"
                        commands=""
                        displayheight="50"
                        displayleft="4"
                        displaytop="325"
                        displaywidth="996"
                        field_weight="799"
                        id=".top:box11."
                        key=".top:box11."
                        original_name="box1"
                        target_field=""
                        uid="desing_form1_1369342922091_cc686538039687f3fe174c6a9b0b99bb9936753e_1369343805083"
                        view="fieldset"
                        style="border-width:0;
                            border-style:None;
                            border-color:transparent;
                            background-color: #e30000">

                        <div
                            binary_variables=""
                            view="div"
                            style="position: absolute; top: 2px; left: 416px; width:164px; height:46px">

                            <div
                                base_class="Classes/ItemsForm/Logo.fx"
                                binary_variables=""
                                displayheight="46"
                                displayleft="416"
                                displaytop="2"
                                displaywidth="164"
                                foundation_type="Classes/Foundation/Image"
                                id=".top:box11:image1."
                                key=".top:box11:image1."
                                kind="instance"
                                original_name="image1"
                                uid="itemsform_dataview1_1368038367945_c486553ba63e77316f04c86372ffc3ae6f0d73b1_1368040829176"
                                uid_to_name=""
                                view="img">

                            </div>

                        </div>

                    </fieldset>

                </div>

                <div
                    binary_variables=""
                    view="div"
                    style="position: absolute; top: 2px; left: 4px; width:996px; height:50px">

                    <fieldset
                        binary_variables=""
                        border-width="0"
                        class="box"
                        commands=""
                        displayheight="50"
                        displayleft="4"
                        displaytop="2"
                        displaywidth="996"
                        field_weight="99"
                        id=".top:box111."
                        key=".top:box111."
                        original_name="box1"
                        target_field=""
                        uid="desing_form1_1369342922091_9f88a2785a5cdfa515e6420db6f725558bb0dafc_1369928873884"
                        view="fieldset"
                        style="border-width:0;
                            border-style:None;
                            border-color:transparent;
                            background-color: #e30000">

                        <div
                            binary_variables=""
                            view="div"
                            align="left" 
                            style="position: absolute; top: 14px; left: 241px; width:522px; height:23px">

                            <div
                                base_class="Classes/ItemsForm/TituloLBL.fx"
                                binary_variables=""
                                displayheight="23"
                                displayleft="241"
                                displaytop="14"
                                displaywidth="522"
                                foundation_type="Classes/Foundation/Label"
                                id=".top:box111:label1."
                                key=".top:box111:label1."
                                kind="instance"
                                labelalign="left"
                                labeltitle="Reportes"
                                original_name="label1"
                                uid="form1_1368027451429_8629d4ae9fc6123c2d53fffd3417127658a7059f_1368038107157"
                                uid_to_name=""
                                view="label">

                            </div>

                        </div>

                        <div
                            binary_variables=""
                            view="div"
                            style="position: absolute; top: 14px; left: 16px; width:197px; height:22px">

                            <input
                                binary_variables=""
                                class="textfield"
                                displayheight="22"
                                displayleft="16"
                                displaytop="14"
                                displaywidth="197"
                                field_type="text"
                                field_weight="99"
                                findable="false"
                                immediate="true"
                                key=".top:box111:textfield1."
                                name=".top:box111:textfield1."
                                original_name="textfield1"
                                size="32"
                                stop_for_input="false"
                                target_field=""
                                type="text"
                                uid="desing_dataview1_1369927531049_9f88a2785a5cdfa515e6420db6f725558bb0dafc_1369929860573"
                                view="text">
                        </div>

                        <div
                            binary_variables=""
                            view="div"
                            style="position: absolute; top: 14px; left: 800px; width:110px; height:22px">

                            <input
                                binary_variables=""
                                class="textfield"
                                displayheight="22"
                                displayleft="800"
                                displaytop="14"
                                displaywidth="110"
                                field_type="text"
                                field_weight="199"
                                findable="false"
                                immediate="true"
                                key=".top:box111:textfield11."
                                name=".top:box111:textfield11."
                                original_name="textfield1"
                                size="18"
                                stop_for_input="false"
                                target_field=""
                                type="text"
                                uid="desing_dataview1_1369927531049_9f88a2785a5cdfa515e6420db6f725558bb0dafc_1369929929174"
                                view="text">
                        </div>

                    </fieldset>

                </div>

                <div
                    binary_variables=""
                    view="div"
                    style="position: absolute; top: 256px; left: 368px; width:138px; height:48px">

                    <div
                        base_class="Classes/ItemsForm/Boton.fx"
                        binary_variables=""
                        command="reportecmd"
                        displayleft="368"
                        displaytop="256"
                        field_type="button"
                        foundation_type="Classes/Foundation/Button"
                        id=".top:ejecutarbtn."
                        key=".top:ejecutarbtn."
                        kind="instance"
                        original_name="button1"
                        uid="form1_1368027451429_8629d4ae9fc6123c2d53fffd3417127658a7059f_1368028211102"
                        uid_to_name=""
                        value="EJECUTAR"
                        view="button">

                    </div>

                </div>

                <div
                    binary_variables=""
                    view="div"
                    style="position: absolute; top: 153px; left: 690px; width:140px; height:22px">

                    <div
                        accell_type="Date"
                        binary_variables=""
                        class="textfield"
                        displayheight="22"
                        displayleft="690"
                        displaytop="153"
                        displaywidth="140"
                        field_type="TEXT"
                        field_weight="399"
                        findable="false"
                        id=".top:fecha1."
                        key=".top:fecha1."
                        kind="date"
                        original_name="textfield11"
                        target_field=""
                        uid="parametros_form1_1369767389242_9aa357ed538e2cd1180f71a52d2b666790b6d2f4_1369767427541"
                        view="text">

                    </div>

                </div>

                <div
                    binary_variables=""
                    view="div"
                    style="position: absolute; top: 209px; left: 690px; width:140px; height:22px">

                    <div
                        accell_type="Date"
                        binary_variables=""
                        class="textfield"
                        displayheight="22"
                        displayleft="690"
                        displaytop="209"
                        displaywidth="140"
                        field_type="TEXT"
                        field_weight="699"
                        findable="false"
                        id=".top:fecha2."
                        key=".top:fecha2."
                        kind="date"
                        original_name="textfield12"
                        target_field=""
                        uid="parametros_form1_1369767389242_9aa357ed538e2cd1180f71a52d2b666790b6d2f4_1369767428554"
                        view="text">

                    </div>

                </div>

                <div
                    binary_variables=""
                    view="div"
                    align="left" 
                    style="position: absolute; top: 127px; left: 112px; width:103px; height:16px">
                    
                    
                    <label
                        binary_variables=""
                        class="label"
                        displayheight="16"
                        displayleft="112"
                        displaytop="127"
                        displaywidth="103"
                        id=".top:label1."
                        key=".top:label1."
                        labelalign="left"
                        labeltitle="Código Proyecto"
                        original_name="label1"
                        uid="parametros_form1_1369767389242_9aa357ed538e2cd1180f71a52d2b666790b6d2f4_1369767438614"
                        view="label"
                        style="font-size: 12px">
                        <nobr>C&oacute;digo Proyecto</nobr>
                    </label>

                </div>

                <div
                    binary_variables=""
                    view="div"
                    align="left" 
                    style="position: absolute; top: 96px; left: 20px; width:177px; height:18px">
                    
                    
                    <label
                        binary_variables=""
                        class="label"
                        displayheight="18"
                        displayleft="20"
                        displaytop="96"
                        displaywidth="177"
                        id=".top:label11."
                        key=".top:label11."
                        labelalign="left"
                        labeltitle="Reporte de Prospectos"
                        original_name="label1"
                        target_field=""
                        uid="desing_form1_1369342922091_cc686538039687f3fe174c6a9b0b99bb9936753e_1369345220656"
                        view="label"
                        style="font-weight: bold;
                            font-family: Verdana;
                            font-size: 14px;
                            color: Black">
                        <nobr>Reporte de Prospectos</nobr>
                    </label>

                </div>

                <div
                    binary_variables=""
                    view="div"
                    align="left" 
                    style="position: absolute; top: 183px; left: 112px; width:109px; height:16px">
                    
                    
                    <label
                        binary_variables=""
                        class="label"
                        displayheight="16"
                        displayleft="112"
                        displaytop="183"
                        displaywidth="109"
                        id=".top:label111."
                        key=".top:label111."
                        labelalign="left"
                        labeltitle="Código Vendedor"
                        original_name="label1"
                        target_field=""
                        uid="parametros_form1_1369767389242_9aa357ed538e2cd1180f71a52d2b666790b6d2f4_1369767439751"
                        view="label"
                        style="font-size: 12px">
                        <nobr>C&oacute;digo Vendedor</nobr>
                    </label>

                </div>

                <div
                    binary_variables=""
                    view="div"
                    align="left" 
                    style="position: absolute; top: 183px; left: 690px; width:70px; height:16px">
                    
                    
                    <label
                        binary_variables=""
                        class="label"
                        displayheight="16"
                        displayleft="690"
                        displaytop="183"
                        displaywidth="70"
                        id=".top:label12."
                        key=".top:label12."
                        labelalign="left"
                        labeltitle="Fecha Final"
                        original_name="label1"
                        target_field=""
                        uid="parametros_form1_1369767389242_9aa357ed538e2cd1180f71a52d2b666790b6d2f4_1369767440103"
                        view="label"
                        style="font-size: 12px">
                        <nobr>Fecha Final</nobr>
                    </label>

                </div>

                <div
                    binary_variables=""
                    view="div"
                    align="left" 
                    style="position: absolute; top: 127px; left: 690px; width:77px; height:16px">
                    
                    
                    <label
                        binary_variables=""
                        class="label"
                        displayheight="16"
                        displayleft="690"
                        displaytop="127"
                        displaywidth="77"
                        id=".top:label13."
                        key=".top:label13."
                        labelalign="left"
                        labeltitle="Fecha Inicial"
                        original_name="label1"
                        target_field=""
                        uid="parametros_form1_1369767389242_9aa357ed538e2cd1180f71a52d2b666790b6d2f4_1369767440522"
                        view="label"
                        style="font-size: 12px">
                        <nobr>Fecha Inicial</nobr>
                    </label>

                </div>

                <div
                    binary_variables=""
                    view="div"
                    align="left" 
                    style="position: absolute; top: 153px; left: 368px; width:199px; height:16px">
                    
                    
                    <label
                        binary_variables=""
                        class="label"
                        displayheight="16"
                        displayleft="368"
                        displaytop="153"
                        displaywidth="199"
                        id=".top:label14."
                        key=".top:label14."
                        labelalign="left"
                        labeltitle="Seleccionar todos los proyectos"
                        original_name="label1"
                        target_field=""
                        uid="parametros_form1_1369767389242_9aa357ed538e2cd1180f71a52d2b666790b6d2f4_1369775854360"
                        view="label"
                        style="font-size: 12px">
                        <nobr>Seleccionar todos los proyectos</nobr>
                    </label>

                </div>

                <div
                    binary_variables=""
                    view="div"
                    align="left" 
                    style="position: absolute; top: 209px; left: 368px; width:212px; height:16px">
                    
                    
                    <label
                        binary_variables=""
                        class="label"
                        displayheight="16"
                        displayleft="368"
                        displaytop="209"
                        displaywidth="212"
                        id=".top:label141."
                        key=".top:label141."
                        labelalign="left"
                        labeltitle="Seleccionar todos los vendedores"
                        original_name="label1"
                        target_field=""
                        uid="parametros_form1_1369767389242_9aa357ed538e2cd1180f71a52d2b666790b6d2f4_1369775880916"
                        view="label"
                        style="font-size: 12px">
                        <nobr>Seleccionar todos los vendedores</nobr>
                    </label>

                </div>

                <div
                    binary_variables=""
                    view="div"
                    style="position: absolute; top: 153px; left: 112px; width:135px; height:21px">

                    <select
                        binary_variables=""
                        class="textfield"
                        displayheight="21"
                        displayleft="112"
                        displaytop="153"
                        displaywidth="135"
                        field_type="SELECT"
                        field_weight="199"
                        findable="false"
                        key=".top:proyecto."
                        name=".top:proyecto."
                        original_name="proyecto"
                        query="SELECT vpy_nombre, vpy_codigo FROM vpy_proyectos ORDER BY vpy_nombre ASC&#59;"
                        size="1"
                        target_field=""
                        uid="parametros_form1_1369767389242_9aa357ed538e2cd1180f71a52d2b666790b6d2f4_1369767424875"
                        view="select"
                        style="width:135px; height: 21px">
                    </select>

                </div>

                <div
                    binary_variables=""
                    view="div"
                    style="position: absolute; top: 256px; left: 528px; width:138px; height:48px">

                    <div
                        base_class="Classes/ItemsForm/Boton.fx"
                        binary_variables=""
                        command="Previous Form"
                        displayleft="528"
                        displaytop="256"
                        field_type="button"
                        foundation_type="Classes/Foundation/Button"
                        id=".top:regresarbtn."
                        key=".top:regresarbtn."
                        kind="instance"
                        original_name="button1"
                        target_field=""
                        uid="itemsform_dataview1_1368038367945_b9a5cdd3a1135a03e03084766ff4a05ae95a25fe_1371071860835"
                        uid_to_name=""
                        value="REGRESAR"
                        view="button">

                    </div>

                </div>

                <div
                    binary_variables=""
                    view="div"
                    style="position: absolute; top: 209px; left: 112px; width:135px; height:21px">

                    <select
                        binary_variables=""
                        class="textfield"
                        displayheight="21"
                        displayleft="112"
                        displaytop="209"
                        displaywidth="135"
                        field_type="SELECT"
                        field_weight="499"
                        findable="false"
                        key=".top:vendedor."
                        name=".top:vendedor."
                        original_name="vendedor"
                        query="SELECT vve_nombre, vve_codigo FROM vve_vendedora ORDER BY vve_nombre ASC&#59;"
                        size="1"
                        target_field=""
                        uid="parametros_form1_1369767389242_9aa357ed538e2cd1180f71a52d2b666790b6d2f4_1369767434464"
                        view="select"
                        style="width:135px; height: 21px">
                    </select>

                </div>

            </div>

        </div>

    </form>
<%=com.unify.nxj.awebServlet.Runtime.getCommandForm(request)%>
</body>

</html>
