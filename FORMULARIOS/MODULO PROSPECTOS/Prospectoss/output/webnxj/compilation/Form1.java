    ...
    extends com.unify.nxj.mgr.NXJForm
{
	...
	ECodigo.assign(us$registerPool.allocateRegister().load("AS"));
	try
	    {
	    while (us$registerPool.allocateRegister().load(counter).ltOp(50).getBooleanValueNullOk())
		{
		    ...
		    try
			{
			us$stmt1 = us$conn1.prepareStatement("SELECT xem_nombre FROM xem_empresas WHERE xem_codigo !=  ?  ORDER BY xem_nombre ASC");
			ECodigo.us$setSqlParameterValue(us$stmt1, 1);
		...
		LMenu.add(ENombre);
		counter.assign(us$registerPool.allocateRegister().load(counter).plusOp(1));
		}
	    }
	...
	public dropdown()
	{
	    super(Form1.this, "dropdown", false, true, 100);
